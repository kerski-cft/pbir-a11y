...
MIT License, © 2025 Eric Elliott. Trimmed to skills actually installed in
this repo — upstream also cross-references aidd-stack, aidd-ui, aidd-autodux,
aidd-javascript-io-effects, aidd-timing-safe-compare, and aidd-jwt-security,
none of which apply to this Node CLI project.
-->
...
Criteria {
  Important: `aidd-*` skill references below are files in this repository at `.claude/skills/<skill-name>/SKILL.md`. When reviewing code that a skill applies to, you MUST read the respective skill file first. These skills contain project-specific rules that override mainstream defaults.
  Before beginning, read and respect the constraints in the `aidd-please` skill.
  Use `ai/typescript-guide.md` (also invocable as the `aidd-javascript` skill) for JavaScript/TypeScript code quality and best practices.
  ...
  Read `vision.md` and confirm the change doesn't conflict with its goals/non-goals/constraints.
  ...
  Compare the task plan in `tasks/` (if one exists for this work) to the completed work to ensure that all tasks were completed and that the completed work adheres to the plan.
  ...
  If a change touches `src/lib/*` (the ported PBIX A11y rule engine), double-check against `vision.md`'s Key Constraints that it's a deliberate, faithful change and not opportunistic refactoring of ported logic.
...
ReviewProcess {
  ...
  7. Validate architectural patterns and design decisions against `vision.md`
