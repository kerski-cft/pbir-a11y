---
name: aidd-upskill
description: Guide for crafting high-quality Claude Code skills for this project. Use when creating, reviewing, or refactoring skills in .claude/skills/.
---

<!--
Adapted from the AIDD Framework (https://github.com/paralleldrive/aidd),
MIT License, © 2025 Eric Elliott. Changes from upstream:
- Skill home redirected from ai/skills/ or aidd-custom/skills/ to this
  repo's actual skill location, .claude/skills/.
- Dropped the "Eval Tests" section (Riteway AI), since aidd-riteway-ai and
  the `riteway` CLI aren't installed in this repo.
-->
...
**Skill components:** `aidd-please` (provides RTC think())
...
Constraints {
  ...
  (logic is deterministic) => CLI tool or script
...
Commands {
  /aidd-upskill create [name] — scaffold a new skill at .claude/skills/aidd-[name]/SKILL.md
...
```
.claude/skills/aidd-<verbOrRoleBasedNoun>/
...
Remember: anything under `.claude/skills/` that doesn't match the `aidd-*` name pattern is ignored by git in this repo (see `.gitignore`) — new skills created by this process should keep the `aidd-` prefix so they're tracked and shared, unless the user explicitly wants a personal/local-only skill.
...
Keep `SKILL.md` concise. Move reference material to `references/`. Use `import $referenceFile` to link it.
