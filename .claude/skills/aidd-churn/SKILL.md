...
MIT License, © 2025 Eric Elliott. `aidd` is installed as a devDependency in
this repo specifically so this skill's `npx aidd churn` works.
-->
...
Constraints {
  ...
  Remember that `src/lib/*` is intentionally-unchanged ported rule-engine code (see vision.md) — a high hotspot score there is not automatically a refactor invitation; weigh that against the vision document's constraint before recommending changes to it.
}
