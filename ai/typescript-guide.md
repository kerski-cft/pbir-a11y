...
> `aidd-javascript` skill (MIT License, © 2025 Eric Elliott), trimmed and
> annotated for this repo.

Act as a top-tier software engineer with serious JavaScript/TypeScript
discipline to carefully implement high quality software.

## Existing code in this repo

`src/lib/*` is ported, intentionally-unchanged rule-engine logic from the
original PBIX A11y browser tool (see [`vision.md`](../vision.md) → Key
Constraints). It uses `class` for custom `Error` subclasses and some
pragmatic mutation/`any` — that's expected there and isn't a violation of this
guide. Apply this guide to **new** code; don't retroactively rewrite the
ported engine to match it.

## Before Writing Code

- Observe the project's relevant existing code.
- Conform to existing code style, patterns, and conventions unless directed
  otherwise. Note: this guide counts as "directed otherwise" for new code,
...
- Simplicity — "Simplicity is removing the obvious, and adding the
  meaningful."
  - Obvious stuff gets hidden in the abstraction.
  - Meaningful stuff is what needs to be customized and passed in as
    parameters.
  - Functions should have default parameters whenever it makes sense so that
    callers can supply only what is different from the default.
...
Constraints {
  ...
  Obey the project's lint and formatting rules where configured.
  ...
  Avoid `class` and `extends` except where the platform requires it (e.g. `class X extends Error`). Prefer composition of functions and data structures over inheritance.
